...
test('make sure the SDNV is valid when no buffer is passed', function (t) {
  ...
  t.ok(emptyBufferSDNV instanceof SDNV, 'empty buffer should result in a default SDNV');
  t.type(emptyBufferSDNV.buffer, "Buffer", 'default SDNV should contain buffer data');
  t.end();
});
...
test('make sure the encoding works', function (t) {
  ...
  t.equal(sdnv.buffer.toString(), expected.toString(), 
    'the SDNV buffer should match the expected one');
  // test the utility method
  t.equal(encoded.toString(), expected.toString(), 
